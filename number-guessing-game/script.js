...
const getDifficulty = () => {
  ...
  do {
    ...
    if (input === "easy") {
      ...
      maxAttempts = 5;
      console.log(
        "💀 GLOOM-9000: Hard mode? I like your confidence. You'll need it."
      );
...
function getPlayerGuess() {
  ...
  do {
    ...
    if (isNaN(guess) || guess < 1 || guess > 100) {
      console.log(
        "💀 GLOOM-9000: Try entering a valid number between 1-100, genius."
      );
      guess = null;
    }
  } while (guess === null);
...
function checkGuess(guess, correctNumber, previousGuess) {
  if (guess === 69) {
    return {
      type: "easter_egg",
      message:
        "🍑 Nice. Real mature. You know what? Game over. I can't deal with this level of immaturity.",
    };
  }
  ...
  const previousDistance =
    previousGuess === null ? null : Math.abs(previousGuess - correctNumber);
...
async function game() {
  ...
  console.log(
    "💀 GLOOM-9000: I've picked a number between 1 and 100. Try to guess it... if you can."
  );
  ...
  console.log(
    `💀 GLOOM-9000: You have ${maxAttempts} attempts. Let's see how badly you fail.`
  );
  ...
  while (attempts < maxAttempts && !hasWon) {
    ...
    if (guess === randomNumber) {
      ...
      console.log(
        `🎉 Impossible! You guessed it in ${attempts} attempt${
          attempts === 1 ? "" : "s"
        }!`
      );
      console.log(
        "💀 GLOOM-9000: I... I can't believe it. You actually won. This is highly irregular."
      );
      updateHighScore(score);
    } else if (attempts === maxAttempts) {
      console.log(
        `💀 GLOOM-9000: FAILURE! You've used all ${maxAttempts} attempts.`
      );
      console.log(
        `💀 GLOOM-9000: The number was ${randomNumber}. Better luck next time... if there is one.`
      );
    } else {
      console.log(
        `💀 GLOOM-9000: ${maxAttempts - attempts} attempt${
          maxAttempts - attempts === 1 ? "" : "s"
        } remaining. Don't waste them.`
      );
    }
  }
  ...
  if (playAgain) {
    console.log(
      "💀 GLOOM-9000: Glutton for punishment, I see. Let's go again!"
    );
    await delay(500);
    game();
  } else {
    console.log(
      "💀 GLOOM-9000: Wise choice. Come back when you've grown a spine... or a brain."
    );
  }
}
